...
def run_scheduled_task_sync(user_phone, bot_username):
    async def async_task_logic():
        ...
        if not api_id or not api_hash:
            logger.error(f"计划任务 User: {user_phone}, Bot: {bot_username} 失败: API ID/Hash 未配置。")
            result = {"success": False, "message": "API ID/Hash 未配置."}
        else:
            logger.info(f"计划任务 (async_task_logic): 开始执行签到任务 User: {user_phone}, Bot: {bot_username}")
            user_config = next((u for u in config['users'] if u['phone'] == user_phone and u.get('status') == 'logged_in'), None)
            if not user_config:
                logger.warning(f"计划任务: 用户 {user_phone} 未登录或不存在，跳过任务。")
                result = {"success": False, "message": "用户未登录或不存在"}
            else:
                session_name = user_config['session_name']
                result = await telethon_check_in(api_id, api_hash, user_phone, session_name, bot_username)
        ...
        logger.info(f"计划任务 (async_task_logic): 任务 User: {user_phone}, Bot: {bot_username} 执行完毕. Result: {result.get('success')}")

    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)

    try:
        logger.info(f"计划任务 (sync_wrapper): 准备在独立事件循环中执行 User: {user_phone}, Bot: {bot_username}")
        loop.run_until_complete(async_task_logic())
        logger.info(f"计划任务 (sync_wrapper): User: {user_phone}, Bot: {bot_username} 的异步逻辑执行完毕。")
    except Exception as e:
        logger.error(f"计划任务 (sync_wrapper) User: {user_phone}, Bot: {bot_username} 执行时发生顶层错误: {e}")
    finally:
        loop.close()
...
def update_scheduler():
    ...
    if config.get('scheduler_enabled'):
        ...
        for task in config.get('checkin_tasks', []):
            ...
            job_id = f"checkin_job_{user_phone}_{bot_username}"

            rand_h, rand_m = get_random_time_in_range(start_h, start_m, end_h, end_m)

            if rand_h is None:
                logger.warning(f"无法为任务 {job_id} 计算随机时间，因时间范围无效。")
                continue
            ...
            try:
                scheduler.add_job(
                    ...
                    args=[user_phone, bot_username],
                    id=job_id,
                    name=f"Checkin {user_phone} with {bot_username}",
                    replace_existing=True
                )
                logger.info(f"已为任务 {job_id} 设置签到时间: {rand_h:02d}:{rand_m:02d}")
            except Exception as e_add_job:
                 logger.error(f"为任务 {job_id} 添加调度时发生错误: {e_add_job}")
    else:
